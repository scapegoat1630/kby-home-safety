package com.kby.home.safety;

/**
 * Created by zhangpeng12 on 2017/5/15.
 * {1：客厅，2：厨房，3：卧室}
 */
public class Constants {
        public static int ROOM_TYPE_LIVINGROOM = 1;

        public static int ROOM_TYPE_KITCHEN = 2;

        public static int ROOM_TYPE_BEDROOM = 3;

        public static int ALERT_TYPE_TEMPERATURE = 1;

        public static int ALERT_TYPE_HUMIDITY = 2;

        public static int ALERT_TYPE_SMOKE = 3;

        public static int ALERT_TYPE_USER_ENTERED = 4;
        public static int ALERT_TYPE_FIRE = 5;
        public static int ALERT_TYPE_WATER = 6;

        public static String[] ROOM_TYPE_NAME = {"","客厅","厨房","卧室"};
        public static String[] ALERT_TYPE_NAME = {"","温度","湿度","烟雾浓度"};

}
